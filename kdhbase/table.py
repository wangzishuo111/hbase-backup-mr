...
class Table(object):
    ...
    def row(self, row, columns=None, timestamp=None, include_timestamp=False,attributes={}):
    ...
    def rows(self, rows, columns=None, timestamp=None,
             include_timestamp=False,attributes={}):
        ...
        if timestamp is None:
            results = self.connection.client.getRowsWithColumns(
                self.name, rows, columns, attributes)
        else:
            if not isinstance(timestamp, Integral):
                raise TypeError("'timestamp' must be an integer")
            ...
            results = self.connection.client.getRowsWithColumnsTs(
                self.name, rows, columns, timestamp, attributes)
    ...
    def cells(self, row, column, versions=None, timestamp=None,
        ...
        if timestamp is None:
            cells = self.connection.client.getVer(
                self.name, row, column, versions, attributes)
        else:
            if not isinstance(timestamp, Integral):
                raise TypeError("'timestamp' must be an integer")
            cells = self.connection.client.getVerTs(
                self.name, row, column, timestamp, versions, attributes)
    ...
    def scan(self, row_start=None, row_stop=None, row_prefix=None,
             ...
             limit=None, sorted_columns=False, reverse=False,attributes={}):
            ...
            if row_stop is None:
                if timestamp is None:
                    scan_id = self.connection.client.scannerOpen(
                        self.name, row_start, columns, attributes)
                else:
                    scan_id = self.connection.client.scannerOpenTs(
                        self.name, row_start, columns, timestamp, attributes)
            else:
                if timestamp is None:
                    scan_id = self.connection.client.scannerOpenWithStop(
                        self.name, row_start, row_stop, columns, attributes)
                else:
                    scan_id = self.connection.client.scannerOpenWithStopTs(
                        self.name, row_start, row_stop, columns, timestamp, attributes)
    ...
    def put(self, row, data, timestamp=None, wal=True,attributes={}):
        ...
        """
        with self.batch(timestamp=timestamp, wal=wal,attributes = attributes) as batch:
            batch.put(row, data)

    def delete(self, row, columns=None, timestamp=None, wal=True,attributes={}):
